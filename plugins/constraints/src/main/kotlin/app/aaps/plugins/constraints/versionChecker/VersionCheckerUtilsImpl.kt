...
class VersionCheckerUtilsImpl @Inject constructor(
    ...
    private fun onNewVersionDetected(currentVersion: String, newVersion: String?): Boolean {
        ...
        if (dateUtil.isAfterNoon() && now > sp.getLong(R.string.key_last_versionchecker_warning, 0) + warnEvery(0)) {
    ...
    private fun onExpireDateDetected(currentVersion: String, endDate: Long) {
        ...
        } else if (dateUtil.isAfterNoon() && now > sp.getLong(R.string.key_last_expired_versionchecker_warning, 0) + warnEvery(endDate)) {
    ...
    private fun shouldWarnAgain() =
        dateUtil.now() > sp.getLong(R.string.key_last_versionchecker_plugin_warning_timestamp, 0) + warnEvery(expiration = sp.getLong(rh.gs(app.aaps.core.utils.R.string.key_app_expiration) + "_" + config.get().VERSION_NAME, 0))
